...
def runGame(ccgame, agents: dict):
    ...
    with open('log.txt', 'a') as f:
        if ccgame.isEnd(state, iter):
            ...
            f.write(f"Winner: Player {winner} at {iter} iterations\n")
            return winner, iter  # return winner
...
def simulateMultipleGames(agents_dict, simulation_times, ccgame):
    ...
    for i in range(simulation_times):
        ...
        if run_result == 1:
            ...
            tie_times += 1
        iter_sum += run_result[1]
        if run_result[1] == 200:
            count_stuck += 1
            iter_sum -= 200
        print('game', i + 1, 'finished', 'winner is player ', run_result)
    iter_avg = iter_sum / (simulation_times - count_stuck)
    with open('log.txt', 'a') as f:
         f.write(f"Average iterations: {iter_avg}\n")
    
    print('Average iterations:', iter_avg)
...
def callback(ccgame):
    ...
    # simulateMultipleGames({1: simpleGreedyAgent, 2: teamAgent}, 50, ccgame)
    simulateMultipleGames({1: teamAgent , 2: simpleGreedyAgent}, 50, ccgame)
    # simulateMultipleGames({1: teamAgent, 2: teamAgent}, 50, ccgame)
